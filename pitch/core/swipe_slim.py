...
def swipe_slim(
    x, Fs=22050, H=256, F_min=55.0, F_max=1760.0, R=10, strength_threshold=0
):
    """
    ...
    L_opt = np.log2(
        Fs * 8 / np.array([F_min, F_max])
    )  # exponents for optimal window sizes 2^L, see paper Section II.G
    L_rnd = np.arange(np.round(L_opt[1]), np.round(L_opt[0]) + 1).astype(
        np.int32
    )  # range of rounded exponents
    ...
    err = np.abs(
        np.log2(8 * Fs / F_coef_log_hz[F_min_idx:F_max_idx]) - np.log2(np.max(N_pow2))
    )
    ...
    for octave, N in enumerate(N_pow2):
        ...
        candidates = (err > octave - 1) & (
            err < octave + 1
        )  # consider pitches +/- 1 octave from current window
    ...
    indeces_shift, _ = parabolic_interpolation(
...
def compute_kernel(f, F_coef_log_hz):
    ...
    prime_numbers = prime_and_one(100)[
        :n_harmonics
    ]  # only consider prime harmonics for kernel peaks
    ...
    for p in prime_numbers:
        ...
        k[main_peak_bins] = np.cos(
            np.dot(
                np.array(2 * np.pi).reshape(-1, 1), ratio[main_peak_bins].reshape(1, -1)
            )
        ).flatten()
        valley_bins = np.logical_and(a > 0.25, a < 0.75)
        k[valley_bins] += (
            np.cos(
                np.dot(
                    np.array(2 * np.pi).reshape(-1, 1),
                    ratio[valley_bins].reshape(1, -1),
                )
            ).flatten()
            / 2
        )
