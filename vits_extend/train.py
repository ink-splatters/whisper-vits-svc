...
from vits_extend.dataloader import create_dataloader_eval, create_dataloader_train
...
def train(rank, args, chkpt_path, hp, hp_str):
    ...
    if os.path.isfile(hp.train.pretrain):
        if rank == 0:
            logger.info(f"Start from 32k pretrain model: {hp.train.pretrain}")
    ...
    if chkpt_path is not None:
        if rank == 0:
            logger.info(f"Resuming from checkpoint: {chkpt_path}")
    ...
    for epoch in range(init_epoch, hp.train.epochs):
        ...
        if rank == 0 and epoch % hp.log.save_interval == 0:
            save_path = os.path.join(pth_dir, "%s_%04d.pt" % (args.name, epoch))
            torch.save(
                {
                    "model_g": (
