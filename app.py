...
class WebUI:
        ...
        with gr.Blocks(theme=gr.themes.Base(primary_hue=gr.themes.colors.green)) as ui:
            ...
            with gr.Tab(i18n("预处理-训练")):
                ...
                with gr.Row():
                    ...
                    self.save_interval = gr.Number(
                        value=5,
                        label="save_interval",
                        info=i18n("检查点保存间隔（epoch）"),
                        interactive=True,
                    )

                    self.keep_ckpts = gr.Number(
                        value=0,
                        label="keep_ckpts",
                        info=i18n("保留最新的检查点文件(0保存全部)"),
                        interactive=True,
                    )

                with gr.Row():
                    self.slow_model = gr.Checkbox(
                        label=i18n("是否添加底模"), value=True, interactive=True
                    )
                ...
                with gr.Row():
                    self.resume_model = gr.Dropdown(
                        choices=sorted(self.name),
                        label="Resume training progress from checkpoints",
                        info=i18n("从检查点恢复训练进度"),
                        interactive=True,
                    )
            ...
            with gr.Tab(i18n("推理")):
                ...
                with gr.Row(), gr.Column():
                    self.keychange = gr.Slider(
                        -24, 24, value=0, step=1, label=i18n("变调")
                    )
            ...
            self.bt_onekey_train.click(
                fn=self.onekey_training,
                inputs=[
                    self.model_name,
                    self.thread_count,
                    self.learning_rate,
            ...
            self.bt_out_model.click(
                fn=self.out_model, inputs=[self.model_name, self.resume_model2]
            )
            self.bt_tb.click(fn=self.tensorboard)
            self.bt_refersh.click(
                fn=self.refresh_model,
                inputs=[self.model_name],
                outputs=[self.resume_model],
            )
            self.bt_resume_train.click(
                fn=self.resume_train,
                inputs=[
                    self.model_name,
                    self.resume_model,
                    self.learning_rate,
    ...
    def preprocessing(self, thread_count):
        ...
        train_process = subprocess.Popen(
            "python -u svc_preprocessing.py -t " + str(thread_count),
    ...
    def out_model(self, model_name, resume_model2):
        ...
        try:
            subprocess.Popen(
                f'python -u svc_export.py -c {self.train_config_path} -p "chkpt/{model_name}/{resume_model2}"',
                stdout=subprocess.PIPE,
            )
            print(i18n("导出模型成功"))
    ...
    def tensorboard(self):
        if sys.platform.startswith("win"):
            ...
            p2 = subprocess.Popen(
                ["grep", "tensorboard"], stdin=p1.stdout, stdout=subprocess.PIPE
            )
            p3 = subprocess.Popen(
            ...
            p4.communicate()
            tb_process = subprocess.Popen(
                "tensorboard --logdir=logs --port=6007", stdout=subprocess.PIPE
            )  # AutoDL端口设置为6007
    ...
    def refresh_model(self, model_name):
        ...
        try:
            ...
            return {"choices": sorted(self.name), "__type__": "update"}
    ...
    def refresh_model2(self, model_name):
        ...
        try:
            ...
            return {"choices": sorted(self.name2), "__type__": "update"}
        except FileNotFoundError:
            return {"label": i18n("缺少模型文件"), "__type__": "update"}
    ...
    def resume_train(
        ...
        train_process = subprocess.Popen(
            f'python -u svc_trainer.py -c {self.train_config_path} -n {model_name} -p "chkpt/{model_name}/{resume_model}"',
            stdout=subprocess.PIPE,
            creationflags=subprocess.CREATE_NEW_CONSOLE,
        )
        while train_process.poll() is None:
    ...
    def inference(self, input, resume_voice, keychange):
        ...
        cmd = [
            ...
            "sovits5.0.pth",
            "--spk",
            f"data_svc/singer/{resume_voice}",
            "--wave",
            "test.wav",
            "--shift",
            keychange,
