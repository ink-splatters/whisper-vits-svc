...
def parse_args():
    ...
    # Required arguments
    parser.add_argument(
        "--audio_files", nargs="+", required=True, help="The audio file to process"
    )
    parser.add_argument(
        "--output_files",
        nargs="+",
    ...
    parser.add_argument(
        "--hop_length", type=int, help="The hop length of the analysis window"
    )

    # Optionally save harmonicity [DEPRECATED]
    parser.add_argument(
        "--output_harmonicity_files", nargs="+", help="The file to save harmonicity"
    )
    # Optionally save periodicity
    parser.add_argument(
        "--output_periodicity_files", nargs="+", help="The files to save periodicity"
    )
    ...
    # Optional arguments
    parser.add_argument(
        "--fmin", default=50.0, type=float, help="The minimum frequency allowed"
    )
    parser.add_argument(
        "--fmax",
    ...
    parser.add_argument(
        "--model", default="full", help='The model capacity. One of "tiny" or "full"'
    )
    parser.add_argument(
        "--decoder",
        default="viterbi",
    ...
    parser.add_argument("--batch_size", type=int, help="The number of frames per batch")
    parser.add_argument("--gpu", type=int, help="The gpu to perform inference on")
    parser.add_argument(
        "--no_pad", action="store_true", help="Whether to pad the audio"
    )
...
def main():
    ...
    if args.embed:
        crepe.embed_from_files_to_files(
            args.audio_files,
            args.output_files,
            args.hop_length,
            args.model,
        ...
        crepe.predict_from_files_to_files(
            args.audio_files,
            args.output_files,
            None,
